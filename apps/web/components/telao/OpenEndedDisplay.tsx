...
function ResponseGrid({
  ...
  return (
    <div
      className="h-full"
      style={{
        columnCount: responses.length <= 2 ? 1 : responses.length <= 6 ? 2 : 3,
        columnGap: 24,
      }}
    >
      <AnimatePresence>
          ...
          >
            ...
            {(r.authorName || (allowVoting && r.voteCount > 0)) && (
              <div className="mt-4 flex items-center justify-between text-base">
                <span style={{ color: subtleColor }}>{r.authorName ?? ''}</span>
                {allowVoting ? (
                  <span className="inline-flex items-center gap-1" style={{ color: subtleColor }}>
